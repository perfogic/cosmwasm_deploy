...
import {
  ...
  PairQuery,
  MappingMetadata,
} from "./TonbridgeBridge.types";
export interface TonbridgeBridgeReadOnlyInterface {
  contractAddress: string;
  owner: () => Promise<String>;
  config: () => Promise<Config>;
  isTxProcessed: ({ txHash }: { txHash: HexBinary }) => Promise<Boolean>;
  ...
  pairMapping: ({ key }: { key: string }) => Promise<PairQuery>;
  sendPacketCommitment: ({ seq }: { seq: number }) => Promise<Uint256>;
  ackCommitment: ({ seq }: { seq: number }) => Promise<Uint256>;
}
export class TonbridgeBridgeQueryClient
  implements TonbridgeBridgeReadOnlyInterface
{
  ...
  pairMapping = async ({ key }: { key: string }): Promise<PairQuery> => {
    return this.client.queryContractSmart(this.contractAddress, {
      pair_mapping: {
        key,
      },
  ...
  sendPacketCommitment = async ({ seq }: { seq: number }): Promise<Uint256> => {
    return this.client.queryContractSmart(this.contractAddress, {
  ...
  ackCommitment = async ({ seq }: { seq: number }): Promise<Uint256> => {
    return this.client.queryContractSmart(this.contractAddress, {
      ack_commitment: {
        seq,
      },
...
export interface TonbridgeBridgeInterface
  extends TonbridgeBridgeReadOnlyInterface {
  contractAddress: string;
  sender: string;
  readTransaction: (
    {
...
export class TonbridgeBridgeClient
  extends TonbridgeBridgeQueryClient
  implements TonbridgeBridgeInterface
{
