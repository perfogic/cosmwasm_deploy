...
export async function getPendingDeposits(
  ...
  let info: PendingDeposit[] = await fetch(`${relayer}/pending_deposits?receiver=${receiver}`).then(
    (res) => res.json()
  );
...
function redeemScript(sigset: SigSet, dest: Buffer) {
  ...
  let truncatedThreshold =
    ((presentVp(sigset) * BigInt(numerator)) / BigInt(denominator)) >> truncation;
...
export async function broadcast(
  ...
  return await fetch(`${relayer}/address?deposit_addr=${depositAddr}&sigset_index=${sigsetIndex}`, {
    method: 'POST',
    body: dest,
  });
}
...
function makeIbcDest(opts: DepositOptions): IbcDest {
  ...
  if (typeof opts.transferTimeoutOffsetSeconds === 'undefined') {
    timeoutTimestampMs = now + oneDaySeconds * 5 * 1000 - (now % (60 * 60 * 1000));
  } else {
