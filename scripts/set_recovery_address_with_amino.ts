...
async function main(): Promise<void> {
  ...
  const { client, address, signer } = await connect(
    mnemonic,
    OraiBtcLocalConfig,
    false
  );
  ...
  const setRecoveryAddressMsg = {
    ...
    value: {
      recovery_address: "tb1qewgfymc9ssrszh7dh202rtsgz3yjzvyyk77vzv",
    },
  };

  const MsgSetRecoveryAddressOriginal = new Type("MsgSetRecoveryAddress").add(
    new Field("recovery_address", 1, "string")
  );
  ...
  client.registry.register(
    setRecoveryAddressMsg.typeUrl,
    MsgSetRecoveryAddressOriginal
  );
  ...
  const { signature } = await (signer as Secp256k1HdWallet).signAmino(
    address,
    signDoc
  );

  const tx = makeStdTx(signDoc, signature);
  const tmClient = await Tendermint37Client.connect(
    OraiBtcMainnetConfig.rpcEndpoint
  );
