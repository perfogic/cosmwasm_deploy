  ...
  for (let i = 0; i < allAccounts.length; i++) {
    if (BigInt(allBalances[i].balance) > 0n && BigInt(allBalances[i].balance) < 28000000n) {
      ...
      console.log(
        ...
        '- BTC amount: ',
        new BigDecimal(BigInt(allBalances[i].balance) * E8).div(new BigDecimal(E14)).toString(),
        'BTC'
      );
      sendObj[allAccounts[i]] = BigInt(allBalances[i].balance) * E8;
    }
  }
  console.log('Total amount', new BigDecimal(total).div(new BigDecimal(E6)).toString());
  console.log(sendObj);
};
