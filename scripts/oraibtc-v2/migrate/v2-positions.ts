...
const start = async () => {
  ...
  const LIQUIDITY_ADDRESS =
    "orai1jd9lc2qt0ltjsatgnu38xsz8ngp89clp0dpeh8geyjj70yvkn4kqmrmh3m";
  const PAIR_ADDRESS =
  ...
  let liquidityTokenInfo = await cosmwasm.queryContractSmart(
    LIQUIDITY_ADDRESS,
    {
      token_info: {},
    }
  ...
  if (
    allAccounts.length !== allBalances.length ||
    allAccounts.length !== rewards.length
  ) {
